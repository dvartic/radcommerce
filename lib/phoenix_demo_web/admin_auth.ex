defmodule PhoenixDemoWeb.AdminAuth do
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_phoenix_demo_web_admin_remember_me"
  ...
  def log_out_admin(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      PhoenixDemoWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_admin:
      ...
      defmodule PhoenixDemoWeb.PageLive do
        ...
        on_mount {PhoenixDemoWeb.AdminAuth, :mount_current_admin}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{PhoenixDemoWeb.AdminAuth, :ensure_authenticated}] do
  ...
  def on_mount(:only_allow_if_none_exists, _params, _session, socket) do
    exists? = Accounts.admin_one_exists()

    if exists? == true do
      {:halt, Phoenix.LiveView.redirect(socket, to: ~p"/admins/log_in")}
    else
      {:cont, socket}
    end
  end
  ...
  def only_allow_if_none_exists(conn, _opts) do
    exists? = Accounts.admin_one_exists()
